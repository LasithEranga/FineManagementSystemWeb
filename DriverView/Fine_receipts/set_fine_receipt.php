...
$query = "SELECT rules_receipt.Ref_no as id, rules_receipt.Date,DATE_ADD(rules_receipt.Date,INTERVAL 14 DAY) as due_date,rules_receipt.officer_id, rules_receipt.time, CAST(rules_receipt.Amount AS DECIMAL(10,2)) AS Amount ,GROUP_CONCAT(rule.description) as rules FROM (SELECT * FROM `fine_receipt` AS fr INNER JOIN rules_broken rb on rb.fine_receipt_id = fr.Ref_No WHERE fr.driver_nic = '".$driver_id."' AND Date >= (CURRENT_DATE-DATE_ADD(CURRENT_DATE,INTERVAL 14 DAY)) AND State = 0 ) as rules_receipt INNER JOIN rule ON rule.rule_id = rules_receipt.rule_id GROUP BY (rules_receipt.Ref_no)";
//echo $query;
$html = "";
$rule_list = "";
